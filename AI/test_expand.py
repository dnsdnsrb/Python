...
class Network():
    ...
    def model(self, x, layer = [28*28, 290, 290, 290, 290, 290, 10]):#[28*28, 450, 300, 200, 100, 50, 10]
        image_size = 32
        output = x
        reshape_size = 0
        layer_num = 0
        ...
        with tf.variable_scope('fc' + str(layer_num)):
            output = fc_layer('fc', output, layer[layer_num], activation='relu', dropout=self.dropout_normal)
            ...
            while step < step_limit:
                ...
                for start, end in zip(range(0, len(train_data), self.batch_size),
                    ...
                    if step == 1000:
                        ...
                        exp_w = tf.constant(0, dtype=tf.float32 ,shape=[784, 100], name='new_w')
