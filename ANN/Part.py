...
class Network():
    ...
    def model(self, x, layer = [28*28, 450, 300, 200, 100, 50, 10]):
        image_size = 32
        output = x
        reshape_size = 0
        # 784, 450, 300, 200, 100, 50
        layer_num = 0


        output = tf.split(output, 28, axis=1)

        layer1 = []
        for i in output:
            layer1.append(tf.layers.dense(i, 196))

        layer2 = []
        for i in layer1:
            layer2.append(tf.layers.dense(i, 196))

        layer3 = []
        for i in layer2:
            layer3.append(tf.layers.dense(i, 196))

        layer4 = []
        for i in layer3:
            layer4.append(tf.layers.dense(i, 196))


        output = tf.concat(layer4, axis=1)
