...
class ACNetwork():
    ...
    def lstm(self, x):
        ...
        rnn_in = tf.expand_dims(output, [0])
    ...
    def train(self):
        ...
        self.apply_grads = self.trainer.apply_gradients(grads, global_vars)
...
class Worker():
    ...
    def game_setting(self, game):
        ...
        game.add_available_buttion(Button.MOVE_LEFT)
        game.add_available_buttion(Button.MOVE_RIGHT)
        game.add_available_buttion(Button.MOVE_ATTACK)
        game.add_available_game_variable(GameVariable.AMM02)
                ...
                while self.env.is_episode_finished() == False:
                    ...
                    if len(episode_buffer) >= 30 and done == False and episode_step_count != max_episode_length - 1:
                        value1 = sess.run(self.local_AC.value,
                                          feed_dict={self.local_AC.inputs:[state],
                                                     self.local_AC.state_in[0]:rnn_state[0],
                                                     self.local_AC.state_out[0]:rnn_state[1]})[0, 0]

                        value_loss, policy_loss, entropy, \
                        grad_norms, var_norms = self.train(episode_buffer, sess, gamma, value1)
                        episode_buffer = []
                        sess.run(self.update_local_ops)
