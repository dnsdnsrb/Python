...
class Network():
    def __init__(self, layer=175):
        ...
        self.x = tf.placeholder("float32", [None, 28*28])
    ...
    def model(self, x):
        ...
        # layer_num += 1
        # with tf.variable_scope('fc' + str(layer_num)):
        #     output4 = fc_layer('fc', output, layer, 1, activation=self.activation, dropout=self.dropout_normal)
        #
        # layer_num += 1
        # with tf.variable_scope('fc' + str(layer_num)):
        #     output5 = fc_layer('fc', output, layer, 1, activation=self.activation, dropout=self.dropout_normal)
