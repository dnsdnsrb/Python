...
class Network():
    ...
    def model(self, x, layer = [3, 32, 64, 128, 256, 375, 10]):
        image_size = 32
        output = x
        reshape_size = 0
        ...
        # layer_num += 1
        # with tf.variable_scope('conv' + str(layer_num)):
        #     output = tf.layers.conv2d(output, layer[layer_num], [5, 5], padding='SAME', activation=tf.nn.relu,
        ...
        with tf.name_scope('reshape'):
            reshape_size = image_size * image_size * layer[layer_num]
            output = tf.reshape(output, [-1, reshape_size])

        layer_num += 1
        with tf.variable_scope('normal' + str(layer_num)):
            output = tf.layers.dense(output, layer[layer_num], activation=tf.nn.relu)

        layer_num += 1
        with tf.variable_scope('normal' + str(layer_num)):
            output = tf.layers.dense(output, layer[layer_num])

        z = output

        layer_num -= 1
        with tf.variable_scope('de_normal' + str(layer_num)):
            output = tf.layers.dense(output, layer[layer_num], activation=tf.nn.relu)
        ...
        with tf.name_scope('reshape'):
            output = tf.reshape(output, [-1, image_size, image_size, layer[layer_num]])

        for i in range(3):
            layer_num -= 1
            with tf.variable_scope('de_conv' + str(layer_num)):
                output = tf.layers.conv2d_transpose(output, layer[layer_num], [2 + i, 2 + i], (2, 2),
                                                    padding='SAME',
                                                    activation=tf.nn.relu)
    ...
    def run(self, step_limit):
        ...
        with tf.Session() as sess:
            ...
            while step < step_limit:
                ...
                summary, loss = sess.run([merged, self.cost],
