...
class Network():
    ...
    # def model_env(self, envs, layers=[200, 150], reuse=False):
    #
    ...
    #             output = tf.layers.dense(output, self.ir_size, activation=None)
    #
    #         env_ir = output #ir
    #
    #         return env_ir

    def model_ir(self, irs, layers=[200, 150, 100], reuse=False):
    ...
    def model(self, env_now):
        ...
        #재귀 사용 가능. 단, loss도 변형해야함
        #env -> ir 변환 과정에서 사용할 네트워크 선택하게 가능(img, txt, voc 등 데이터에 따른 적합한 네트워크)
        #여기선 now와 obj가 동일한 형태라서 같은 네트워크를 사용함
        #
        # self.ir_now = self.model_env(env_now)
        # self.ir_obj = self.model_env(env_obj, reuse=True)
        #
        # self.irs = tf.concat([self.ir_now, self.ir_obj], 0)
        #
        # ir_, act = self.model_ir(self.irs)


        # self.ir_now = self.model_env(env_now)
        # self.irs = self.ir_now

        return self.model_ir(env_now)

    def train(self):

        self.pred, self.act = self.model_ir(self.env_now)

        # for i in range(100):        #재귀적으로 나온 act를 묶어서 self.act로 삼는다. => predict에서 여러 act가 나옴.
        #     act = self.model_ir(self.env_now)
        #     self.act = tf.concat([self.act, act], 0)
    ...
    # def env2ir(self, session, state):        #model_env만 실행
    #     irs = session.run(self.irs, feed_dict={self.env_now: state, self.done: None})
    #     return irs
