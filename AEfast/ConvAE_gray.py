...
def model(X):                   #32 32 3             #in        out                                                              #28 28
    ...
    output = tf.reshape(X, [-1, 32, 32, 1])

    with tf.variable_scope('en_conv1'):
        output = conv_layer('en_w1', output, [5, 5, 1, 32])                  #16 16  14 14
...
X = tf.placeholder("float", [None, 32, 32], name='X')
...
def run(epochs):
    with tf.Session() as sess:
        ...
        print(x_train.shape)
        clf = svm.SVC(max_iter=500, random_state=3)
        ...
        #     im = Image.fromarray(arr_uint[i])
        #     im.save(image_path)

        #result(sess, teX, "Image_gray2gray")
